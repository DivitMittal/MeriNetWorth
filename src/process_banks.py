...
from src.config import BANK_PATH, DATA_PATH, OUTPUT_PATH
...
def parse_equitas_statement(file_path):
    try:
        ...
        return {
            "bank": "Equitas",
            "account_number": account_number or file_path.stem,
            "holder_name": holder_name,
            "balance": closing_balance,
            "source_file": file_path.name,
        }
...
def export_to_excel(accounts_df):
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        summary = (
            accounts_df.groupby("bank")
            .agg({"balance": "sum", "account_number": "count"})
